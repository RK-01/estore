      ...
      <div className="header-container">
        ...
        <div className="rk-flex-row-container">
          ...
          {displayCartSummary && 
            ...
            <div className="cart-content-header"><h3>Cart Total </h3><p>Rs. {cartItems.reduce((acc, item)=> acc + Number(item.qty) * item.price, 0)}</p></div>
            {cartItems.map(item=>{
            ...
            <div className="cart-summary-btns">
              <button className="rk-btn-close" onClick={()=>hideCartSummary()}>Close</button>
              <Link to={`/cart`} className="rk-btn"><span>Go to cart</span></Link>
            </div>
            </div>
)
          }
