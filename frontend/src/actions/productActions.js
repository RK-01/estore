...
export const listProductsByCategory = (categoryId, pageNumber ='') => async (dispatch) => {
    console.log("category = "+categoryId)
try{
    dispatch({type: PRODUCT_LIST_BY_CATEGORY_REQUEST})
    const {data} = await axios.get(`/api/products/${categoryId}/category?pageNumber=${pageNumber}`)
...
export const addCategoryToProduct = (productId, category) => async (dispatch, getState) => {
    try {
        dispatch({
            type: PRODUCT_ADD_CATEGORY_REQUEST
        })
        const {
            userLogin: {userInfo}
        } = getState()
        ...
        await axios.post(`/api/products/${productId}/addCategoryToProduct`, {category}, config)
        dispatch({
